...
class CartRepository {
  public static function add_to_cart($db, $uid, $pid, $qty, $attributes) {
    if ($attributes == NULL) {
      $query = 'UPDATE carts SET quantity=quantity+:qty, date_modified=NOW()
    ...
    if ($stmt->rowCount() == 0) {
      $query = 'INSERT INTO carts (user_id, product_id, quantity, attributes, date_created)
  ...
  public static function remove_from_cart($db, $uid, $pid, $attributes) {
    if ($attributes == NULL) {
      $query = 'DELETE FROM carts WHERE user_id=:uid AND product_id=:pid AND attributes IS NULL;';
    } else {
      $query = 'DELETE FROM carts WHERE user_id=:uid AND product_id=:pid AND attributes=:attributes;';
    }
  ...
  public static function clear_cart($db, $uid) {
  ...
  public static function get_shopping_cart_contents($db, $uid) {
    $query = '
      SELECT
        cat.name AS cname,
