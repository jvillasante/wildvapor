<div class="container-2">
  ...
  <div class="prod">
    <div class="clearfix">
      <img src="<?php echo $this->uploads_big($kit['image']); ?>" style="border: 4px solid #e5e5e5;">
      <?php include BASE_URI . DS . 'views' . DS . 'includes' . DS . 'rating.php'; ?>
    </div>
    </div><!--end:prod-->
    <div class="prod-detail">
      <h2><?php echo $kit['name']; ?></h2>
      <?php echo \Helpers\Util::get_price($kit['price'], $kit['sale_price']); ?>
      <br /><br />

      <h3>Products that come with this Kit</h3>
      <form method="post" action="<?php echo $this->url('/shop/add_product'); ?>">
        <fieldset>
          <?php foreach ($products as $p) { ?>
            <div class="kit-product">
              <input type="hidden" value="<?php echo $p['quantity'] . ' x ' . $p['name']; ?>" name="products[product_<?php echo md5($p['id']); ?>]" class="span1">
              <img class="kit-image-product" width="100" height="100" src="<?php echo $this->uploads_thumb($p['image']); ?>" style="border: 4px solid #e5e5e5;">
              <h4><?php echo $p['name']; ?></h4>
              <p><?php echo $p['description']; ?></p>
              <?php if (isset($p['attributes'])) { ?>
                Choose: &nbsp;
                <?php \Helpers\Form::create_attributes_for_kit($p['attributes'], 'product_' . md5($p['id']) . '_attr_'); ?>
              <?php } ?>
            </div>
          <?php } ?>
     ...
     <div id="tab">
      ...
      <div class="list-wrap">
        <div id="details">
          <p><?php echo $kit['description']; ?></p>
        </div>
        <ul id="specs" class="hide">
          <li><span>Product Code:</span> <?php echo $kit['id']; ?></li>
          <li><span>Availability:</span> <?php echo \Helpers\Util::get_stock_status($kit['stock']); ?></li>
          <li><span>Price:</span> $<?php echo number_format($kit['price'], 2); ?></li>
          <li><span>Sale Price:</span> $<?php echo \Helpers\Util::get_just_price($kit['price'], $kit['sale_price']); ?></li>
  ...
  <div class="feedback-wrap">
    <div class="dvision">
      <?php if (isset($reviews) && count($reviews)) { ?>
        <h3><?php echo $reviews_count; ?> Reviews &mdash; <span><a href="#"><?php echo $kit['name']; ?></a></span></h3><br>
